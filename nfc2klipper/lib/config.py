...
class Nfc2KlipperConfig:
    ...
    def get_config(cls, config_dir: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        config: Dict[str, Any] = {
            "webserver": {
                "disable_web_server": os.getenv("N2K_WEB_DISABLE", "").lower()
                in ("1", "true", "yes"),
            ...
            "spoolman": {
                "spoolman-url": os.getenv("N2K_SPOOLMAN_URL", "http://localhost:8000"),
            },
            "moonraker": {
                "moonraker-url": os.getenv("N2K_MOONRAKER_URL", "http://localhost"),
                "clear-spool": os.getenv("N2K_CLEAR_SPOOL", "").lower()
                in ("1", "true", "yes"),
                "always-send": os.getenv("N2K_ALWAYS_SEND", "").lower()
                in ("1", "true", "yes"),
            },
            "macros": {
        ...
        if file_config:
            ...
            cls._merge_section(
                ...
                file_config,
                "webserver",
                ["disable_web_server", "web_address", "web_port", "socket_path"],
                "N2K_WEB_",
            )
            cls._merge_section(config, file_config, "nfc", ["nfc-device"], "N2K_NFC_")
            cls._merge_section(
                config, file_config, "spoolman", ["spoolman-url"], "N2K_SPOOLMAN_"
            )
            cls._merge_section(
                config,
                file_config,
                "moonraker",
                ["moonraker-url", "clear-spool", "always-send"],
                "N2K_MOONRAKER_",
            )
            cls._merge_section(
                config,
                file_config,
                "macros",
                ["setting_gcode", "clearing_gcode"],
                "N2K_",
            )

            if "opentag3d" in file_config:
                ot3d = file_config["opentag3d"]
                if "filament_name_template" in ot3d and not os.getenv(
                    "N2K_FILAMENT_TEMPLATE"
                ):
                    config["opentag3d"]["filament_name_template"] = ot3d[
                        "filament_name_template"
                    ]
                config["opentag3d"]["filament_field_mapping"] = ot3d.get(
                    "filament_field_mapping", {}
                )
                config["opentag3d"]["spool_field_mapping"] = ot3d.get(
                    "spool_field_mapping", {}
                )
