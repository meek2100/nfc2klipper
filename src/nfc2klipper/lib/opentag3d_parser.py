...
class OpenTag3DParser:
    ...
    def parse(
        ...
        vendor = self.spoolman_client.find_vendor_by_name(tag_data["manufacturer"])

        if vendor is None:
            logger.info("Creating new vendor: %s", tag_data["manufacturer"])
            vendor = self.spoolman_client.create_vendor(tag_data["manufacturer"])
            if vendor is None:
        ...
        filament = self.spoolman_client.find_filament(
            vendor_id, filament_name, material
        )
        ...
        if "extra" not in spool_data:
            spool_data["extra"] = {}
        spool_data["extra"]["nfc_id"] = identifier.lower()
